...
from .objects import *
from .get_product_cathegory import get_product_cathegory
...
def process_receit_from_img(user_id, basic_info, product_list):
    ...
    if transaction is None:
        transaction = Transaction(user_id=user_id, location=store, amount=basic_info['total_price_tag'],
    ...
    for product in product_list:
        purchase = Purchase(category = get_product_cathegory(product['product_name'], product['product_price']),
                            date=timestamp, store=store,
                            name=product['product_name'], figure=product['product_price'], amount=product['product_quantity'])
